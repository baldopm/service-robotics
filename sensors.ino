...
void initializeSensors() {
  pinMode(US_LEFT, INPUT); //Ultrasound sensor Left
  pinMode(US_RIGHT, INPUT); //Ultrasound sensor Right
  pinMode(US_FRONT, INPUT); //Ultrasound sensor Front
  pinMode(interruptPin, INPUT_PULLUP); //Interrupt sensor
  attachInterrupt(digitalPinToInterrupt(interruptPin), cylinder_found, RISING);
}
...
void processSensors() {
  ...
  if(senseValue(US_FRONT) > STOP_THRESHOLD && analogRead(US_LEFT) > STOP_THRESHOLD && analogRead(US_RIGHT) > STOP_THRESHOLD && lastAction == ACTION_MOVE) {
    Serial.println("STOP");
    Serial.println(col_distance);
    enabled = false;
    motorStop(true);
  }
  ...
  if(sense_time - last_sense_time2 > 700) {
    ...
    int left_cur = analogRead(US_RIGHT);
    int right_cur = analogRead(US_LEFT);
    ...
    if(left_prev + right_prev > 0 && (left_diff > SIDE_OPENING_THRESHOLD || right_diff> SIDE_OPENING_THRESHOLD)) {
      //Serial.print("WALL APPEARED ON ONE OF THE SIDES!!!!");
      //enabled = false;
    }
...
void sense(boolean &north, boolean &east, boolean &south, boolean &west) {
  ...
  if(corner == UNDEFINED && (!east || !west)) {
    corner = !east ? 3 : 1;
    Serial.print("WE ARE IN THE " + String(corner == 1 ? "RIGHT" : "LEFT") + " CORNER!");
  ...
  if(corner != UNDEFINED) {
    ...
    if(DEBUG & 2) {
      Serial.print("STORING WALL RESULTS FOR " + String(row) + "/" + String(col) + "...");
    }
  ...
  if(DEBUG_PRINT & 2) {
    ...
    startGroup("SENSOR DEBUG");
      Serial.print("TURNS " + String(turns));
      Serial.print( "NORTH INDEX " + String( (0 + shift) % 4 ) );
      Serial.print( "EAST INDEX " + String( (1 + shift) % 4 ) );
      Serial.print( "SOUTH INDEX " + String( (3 + shift) % 4 ) );
      
      startGroup("RELATIVE");
        Serial.print(String(values[0]));
        Serial.print(String(values[1]));
        Serial.print(String(values[3]));
      endGroup();
              
      startGroup("ABSOLUTE");
        if(north) { Serial.print("WALL TO THE NORTH"); }
        if(east) { Serial.print("WALL TO THE EAST"); }
        if(south) { Serial.print("WALL TO THE SOUTH"); }
        if(west) { Serial.print("WALL TO THE WEST"); }
      endGroup();
    endGroup();
  }
          
  // Floodfill
  floodfill(toRow, toCol);
}
...
int senseValue(int pin) {
  ...
  //if(pin == US_FRONT && !use_front) {
  //  return (readSensorAnalog(US_LEFT) + readSensorAnalog(US_RIGHT)) / 2;
  //}
